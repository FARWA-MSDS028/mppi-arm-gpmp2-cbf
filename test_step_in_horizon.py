"""
test_step_in_horizon.py
Runs the real threaded pipeline for 15 seconds and prints goal_err
over time, to check whether the fix actually lets the robot make
steady progress now, instead of staying stuck near 1.73.
"""
import time
import numpy as np
import mujoco
...
shared.set_start_execution()  # skip the demo pause -- this is a test, start moving immediately

print("Running for 90 seconds...")
worst_h_so_far = float("inf")
for i in range(90):
    time.sleep(1)
    snap = state.snapshot()
    h_val = snap['safety_margin']
    if h_val is not None:
        worst_h_so_far = min(worst_h_so_far, h_val)
    print(f"t={i+1}s  goal_err={snap['goal_error']}  h(x)={h_val}  "
          f"worst_h_so_far={worst_h_so_far:.4f}  gpmp2_v={snap['gpmp2_version']}")
